<script lang="ts">
	...
	$: {
		let nextLesson = lessons
			.sort((a, b) => a.order - b.order)
			.find((lesson) => isTimeGreaterThan(new Date(lesson.startTime), new Date(startTime)));
		freeUntil = lessons.length > 0 && nextLesson ? nextLesson.startTime : -1;
	}
</script>

<a
	href={getHrefLink(room, PlanType.ROOM, startTime)}
	class="bg-display flex w-full items-center gap-3 rounded-lg p-3"
>
	...
	<div>
		...
		{#if freeUntil !== -1}
			<p class="text-disabled text-sm">
				Dieser Raum ist frei bis: {format(new Date(freeUntil), 'HH:mm')} Uhr
			</p>
		{/if}
	</div>
</a>
